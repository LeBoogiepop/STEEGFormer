...
class MetricLogger(object):
    ...
    def synchronize_between_processes(self):
        #print("Starting synchronization", flush=True)
        for meter in self.meters.values():
            meter.synchronize_between_processes()
        #print("Finished synchronization", flush=True)
...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self):
        self._scaler = torch.amp.GradScaler("cuda")#torch.cuda.amp.GradScaler()
    ...
    if args.resume:
        if args.resume == "latest":
            ...
            def parse_epoch(path):
                ...
                try:
                    # filename is 'checkpoint-{epoch}.pth'
                    return int(fname.split("-")[1].split(".pth")[0])
                except:
                    return -1
            # pick the checkpoint with the largest epoch
